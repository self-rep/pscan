...
func checkport(ip string, port int, ms time.Duration) {
	...
	fmt.Println("[PSCAN] Found:", ip, ":", port)
	conn.Close()
...
func main() {
	...
	fmt.Println("Windows Version! No Color Sadly")
	fmt.Println("[NOTICE] Scanning Ip: "+ip+" With Threads(", threads, ") And Timeout(", ms, ")")
	...
	tn := time.Since(tm)
	fmt.Println("[PSCAN] Finished Scan")
